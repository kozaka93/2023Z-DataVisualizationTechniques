...
mtcars[1,]
mtcars[,1] #wektor w wyniku
mtcars[,1, drop = FALSE] #ramka danych a nie wektor w wyniku
mtcars[1,1]
...
mtcars[1,2] #to wybiera komurke jedną
mtcars[c(1,2)] #to pierwszą i drugą kolumne
...
mean(mtcars$drat[mtcars$cyl == 4]) #równoważe
...
for ( x in top1000passwords){
  result <- proton(action = "login", login="johnins", password= x)
