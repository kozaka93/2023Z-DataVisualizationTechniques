...
ggplot() + 
  ...
  coord_map("mercator") + 
  theme_bw() + 
  ggtitle("Emisje CO2 na mieszkańca w roku 2020") +
  guides(fill = guide_legend(title = "Roczne emisje CO2\n(tony per capita)")) +
  theme(axis.text.x = element_blank(), axis.ticks.x = element_blank(), 
        axis.text.y = element_blank(), axis.ticks.y = element_blank(), 
        legend.position = "bottom", plot.title = element_text(size = 21)) + 
  scale_fill_distiller(palette = "YlOrBr", limits = c(0, 22)) + 
  xlab(" ") + ylab(" ") + 
  geom_point(data = points, aes(x = long, y = lat), color = "white", size = 3) +
  geom_text(data = points, aes(x = long, y = lat, label = names), color = "black")
