...
grid <- expand.grid(
  `MeanUnderweight` = seq(
    min(processed_data$MeanUnderweight),
    max(processed_data$MeanUnderweight),
    length.out = 100
  ),
  `MeanOverweight` = seq(
    min(processed_data$MeanOverweight),
    max(processed_data$MeanOverweight),
    length.out = 100
  )
)
grid$Continent <-
  knn(
    train = processed_data[, c("MeanUnderweight", "MeanOverweight")],
    test = grid,
    cl = processed_data$Continent,
    k = 3
  )
...
plot_bmi <- ggplot() +
  geom_point(
    data = processed_data,
    aes(x = `MeanUnderweight`, y = `MeanOverweight`, color = `Continent`),
    size = 2
  ) +
  ...
  labs(
    ...
    subtitle = "Perentage of children suffering from these conditions. Ages 0 to 5."
  ) +
  theme(
